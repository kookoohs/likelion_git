...
const storage=multer.diskStorage({
  destination:(req,file,done)=>{
    done(null,'./public/image');   // 이 위치에
  },filename:(req,file,done)=>{
    done(null,file.originalname);   // 원본 이름으로 저장
  }, //limit:5*1024*1024    // DOS 공격을 위해 파일 사이즈 제한
});
...
/////// 파일 첨부 처리
let imagepath='';
// req: binary->text 변환된 상태, 우리는 binary 파일이 필요 -> multer
...
router.post("/post/save", async (req, res) => {
  ...
  if (req.session.user) {
    if (req.session.user.userid == req.body.id) {
      ...
      // crsf 토큰이 존재하고 req에도 csrf 토큰이 존재하고 그 둘이 같다면
      if (typeof req.session.csrf_token != 'undefined' &&  typeof req.body.frsc != 'undefined' &&  req.session.csrf_token==req.body.frsc) { //csrf_token이 맞으면
        ...
        .then((result) => {
          ...
          delete req.session.csrf_token;    // 데이터가 저장된다면 csrf 토큰 삭제
          list(mongodb, req, res);
        });
      } else {
